...
const IpadView = () => {
	...
	useEffect(() => {
		const fetchData = async () => {
			try {
				const promise = await OffchainAPI.httpGet(
					'/api/metrics/urm_platform/daily'
				);
				const promiseTvl = await OffchainAPI.httpGet(
					'/api/get-main-metrics'
				);
